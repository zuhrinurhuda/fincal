...
export default function AmountInput({ config, value, onChange, error }: Props) {
  const id = useId();
  ...
  return (
    <div>
      ...
      <div
        className={`flex items-center rounded-lg border bg-white dark:bg-gray-800 transition-colors ${
          error
      ...
      >
        {config.prefix && (
          <span className="shrink-0 pl-3 pr-2 text-sm text-gray-500 dark:text-gray-400 select-none">
            {config.prefix}
          </span>
        )}
        ...
        <input
          ...
          placeholder="0"
          aria-describedby={
            [error ? errorId : "", config.helpText ? helpId : ""]
              .filter(Boolean)
              .join(" ") || undefined
          }
          aria-invalid={error ? "true" : undefined}
          className="flex-1 min-w-0 py-2.5 px-3 bg-transparent text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none"
        />

        {config.suffix && (
          <span className="shrink-0 pr-3 pl-2 text-sm text-gray-500 dark:text-gray-400 select-none">
            {config.suffix}
          </span>
        )}
      </div>
